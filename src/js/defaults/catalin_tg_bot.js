            ...
            (value, index, array) => {
                const
                    ...
                    label = index === 0 ? `до ${years}` : (
                        array[index + 1] ?
                            `от ${getCategories()[this.state.selectedSex || 1][array[index - 1]] + 1} до ${years
                            }` : `от ${years}`
                    )
                    ;
...
export async function getRegDate(user_id) {
    const data = await fetch(proxyUrl + `https://api.pxolly.ru/m/users.getRegisteredDate?user_ids=${user_id}`).then(r => r.json());

    if (data && data.response && data.response[0] && data.response[0].registered && data.response[0].id == user_id) {
        return new Date(data.response[0].registered * 1000);
    } else {
        throw new Error('No date: ' + JSON.stringify(data));
    }
}