...
const
    ...
    redesign = false, // новая версия фиолетовый цвет
    removeAds = true,

    allApprove = true // прожать все галочки соглашения
...
class VKTime extends React.Component {
    ...
    constructor(props) {
        ...
        this.state = {
            ...
            autoSaveAlbum: false,
            checkbox_save_photo_album: allApprove,
            checkbox_agreement: allApprove
        };
                            ...
                            onClick={async () => {
                                try {
                                    ...
                                    if (savedDataJSON) {
                                        ...
                                        let err = false;
                                        try {
                                            const regDate = await getRegDate(getUrlParams().vk_user_id);
                                            const
                                                hours = Math.round(
                                                    (Date.now() - regDate) / 1000 / 60 / 60 / 9.5
                                                ),
                                                days_lost = (Date.now() - regDate) / 1000 / 60 / 60 / 24,
                            ...
                            </div>
                            {
                                withPolitical && <React.Fragment>
                                    <Checkbox
                                        ...
                                        checked={this.state.checkbox_save_photo_album}
                                    >
                                    ...
                                    <Checkbox
                                        ...
                                        checked={this.state.checkbox_agreement}
                                    >
                                        ...
                                        (value, index, array) => {
                                            const
                                                ...
                                                label = index === 0 ? `от ${years}` : (
                                                    array[index + 1] ?
                                                        `от ${this.getCategories()[this.state.selectedSex || 1][array[index - 1]] + 1} до ${years
                                                        }` : `от ${years}`
                                                )
                                                ;
            ...
            for (let i = 1; i <= 5; i++) {
                try {
                    ...
                    const
                        ...
                        days = Math.floor(hours / 24),
                        messages = friends > 0 ? (hours * friends * 2) : (hours * 2),
                        messages_in_day = Math.floor(messages / ((Date.now() - regDate) / 1000 / 60 / 60 / 24)),
